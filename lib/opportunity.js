...
function scoreOpportunity(item={},options={}){
  ...
  const marginScore=marginEstimate>=15?20:marginEstimate>=8?12:5;  const supplierScore=hasSupplier?clamp((marginEstimate/price)*20,0,20):0;  const observedSold=Math.max(0,Number(item.sold)||0);  const demandScore=observedSold>0?clamp(Math.log10(observedSold+1)*7,0,15):0;  const totalMax=hasSupplier?120:100;
...
function extractTrendTerms(title=""){
  ...
  return String(title).toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\\s+/).filter(w=>w.length>=4&&!stop.has(w));
}
...
function compareTrendSnapshots(previous=[],current=[]){
  const oldMap=new Map((Array.isArray(previous)?previous:[]).map(x=>[x.term,{listings:Number(x.listings)||0,avgPrice:Number(x.avgPrice)||0}]));
  return (Array.isArray(current)?current:[]).map(x=>{
    const before=oldMap.get(x.term);
    const now=Number(x.listings)||0;
    const old=before?.listings||0;
    const change=now-old;
    const pct=old>0?(change/old)*100:(now>0?100:0);
    return {...x,previousListings:old,listingChange:change,changePercent:Number(pct.toFixed(1)),direction:change>0?"Rising":change<0?"Falling":"Stable"};
