...
function scoreOpportunity(item={},options={}){
  ...
  const supplierCost=Math.max(0,Number(item.supplierCost)||0);\n  const shipping=Math.max(0,Number(item.shipping)||0);\n  const hasSupplier=supplierCost>0;\n  const marginEstimate=hasSupplier?price-(price*0.13)-supplierCost-shipping:price*0.87;
  const marginScore=marginEstimate>=15?20:marginEstimate>=8?12:5;\n  const supplierScore=hasSupplier?clamp((marginEstimate/price)*20,0,20):0;\n  const observedSold=Math.max(0,Number(item.sold)||0);\n  const demandScore=observedSold>0?clamp(Math.log10(observedSold+1)*7,0,15):0;\n  const totalMax=hasSupplier?120:100;
  const rawScore=priceScore+competitionScore+seasonScore+titleScore+marginScore+supplierScore+demandScore;\n  const score=Math.round(clamp((rawScore/totalMax)*100));
  ...
  if(marginScore>=12) reasons.push(hasSupplier?"Positive estimated supplier margin":"Room for marketplace fees before supplier cost");\n  if(demandScore>0) reasons.push("Observed sold-demand signal available");
