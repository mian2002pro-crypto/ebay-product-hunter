...
function normalizeCjProducts(payload) {
  ...
  const rawRows = Array.isArray(data.content) ? data.content : [];
  const rows = rawRows.flatMap((row) => Array.isArray(row?.productList) ? row.productList : [row]);
  return {
    total: Number(data.totalRecords) || rows.length,
    items: rows.map((item) => ({
      id: item.pid || item.id || item.productSku || item.vid || "",
      title: item.productNameEn || item.nameEn || item.productName || "Untitled supplier product",
      price: parsePrice(item.sellPrice ?? item.nowPrice),
      currency: "USD",
      source: "CJdropshipping",
      sku: item.productSku || item.sku || item.spu || "",
      image: item.bigImage || "",
